...
--
-- Host: localhost
-- Generation Time: Apr 26, 2014 at 12:40 PM
...
(4, 6, 'uploads/shobhit1.txt', 'shobhit1.txt'),
(4, 7, 'uploads/shobhit2.txt', 'shobhit2.txt'),
(5, 8, 'uploads/harshil.txt', 'harshil.txt'),
(6, 9, 'uploads/jha.txt', 'jha.txt'),
(15, 10, 'uploads/mayank.txt', 'mayank.txt'),
(18, 11, 'uploads/luhadia.txt', 'luhadia.txt'),
(19, 12, 'uploads/shreyas.txt', 'shreyas.txt');
...
(3, 45, '7.00', 'windows phone Developers', 'Y', '2014-04-26 10:35:53', 1, '2014-04-08 00:00:00'),
...
INSERT INTO `slots` (`slot_id`, `room_no`, `start_time`, `end_time`) VALUES
(1, '2201', '2014-08-15 00:00:00', '2014-07-16 00:00:00'),
(2, '1101', '2014-07-31 03:24:19', '2014-07-31 08:00:00');
...
--
-- Stand-in structure for view `slot_time`
--
...
--
-- Structure for view `slot_time`
--
DROP TABLE IF EXISTS `slot_time`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `slot_time` AS (select `s`.`slot_id` AS `slot_id`,`s`.`room_no` AS `room_no`,`s`.`start_time` AS `start_time`,`s`.`end_time` AS `end_time`,`sa`.`j_id` AS `j_id`,`sa`.`c_id` AS `c_id` from (`slot_alloted` `sa` join `slots` `s`) where (`s`.`slot_id` = `sa`.`slot_id`));
